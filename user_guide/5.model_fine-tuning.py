...
def cross_validate(X, y, model_class, num_folds=10, epochs=100, lr=0.0001):
...
if __name__ == "__main__":
    ...
    def get_auc_list(list1, list2, filename):
        with open(filename, 'w')as f:
            for i in range(len(list2)):
                f.write(str(list1[i][0]) + '\t' + str(list2[i][0]) + '\t'+name_list[i]+'\n')


    get_auc_list(label.detach().numpy().tolist(), value.detach().numpy().tolist(), 'fine_tuning_result.txt')
    ...
    pre_list1, pre_list2 = getlist('result.txt')
    fpr5, tpr5, threshold5 = roc_curve(pre_list1, pre_list2)
    for i in range(len(fpr5)):
        print(threshold5[i], '\t', str(1 - fpr5[i]))

    for i in range(len(fpr5)):
        if 1 - fpr5[i] < 0.99:
            break
    auc = roc_auc_score(pre_list1, pre_list2)
